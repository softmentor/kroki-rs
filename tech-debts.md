...
- [x] **TD-08**: `WebpQuality` accepted but ignored (`image_converter.rs`) — *Lossy falls back to lossless with warning; needs `webp` crate for true lossy*
- [x] **TD-09**: Cache dir resolution duplicated 3× (`cli`, `font_manager`)
- [x] **TD-10**: `cmd.rs` hardcodes `-Tsvg` ignoring format param
- [x] **TD-11**: `validate()` is no-op on every provider
- [x] **TD-12**: Verbose per-tool debug logging in `capabilities.rs`
- [x] **TD-18**: VegaLite pipeline errors don't identify which stage failed (`vega.rs`)
- [x] **TD-22**: Unknown type (400) conflated with tool-not-installed (503) (`handlers.rs`)
...
- [x] **TD-29**: `test_decode_debug` uses println, never asserts (`utils/mod.rs`)
