...
Proposed tool concept: a standalone Rust CLI (example name: `forgeflow`) that abstracts CI/container/caching complexity while preserving local developer UX.

Design goals:

- One command surface across projects (`forgeflow setup`, `forgeflow verify`, `forgeflow ci plan`, `forgeflow release`).
...
- Phase A: treat current `dflow` as compatibility facade and map it to `forgeflow` commands.
